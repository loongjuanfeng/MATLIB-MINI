...
void assert_matrix_near(const Matrix& mat,
    ...
    assert_shape(mat, expected.size(),
                 expected.empty() ? 0 : expected[0].size(),
                 message + " shape mismatch");
    for (std::size_t i = 0; i < expected.size(); ++i) {
        for (std::size_t j = 0; j < expected[i].size(); ++j) {
            assert_near(mat(i, j), expected[i][j], tol,
...
int main() {
    ...
    tr.run("Rational arithmetic and comparisons", [] {
        ...
        assert_true((a * b) == Rational(1, 6),
                    "Rational multiplication failed");
    ...
    tr.run("String output, print and stream operator", [] {
        ...
        assert_true(s.find("1.00") != std::string::npos,
                    "str precision failed");

        std::ostringstream oss;
        oss << A;
        assert_true(oss.str().find("[") != std::string::npos,
                    "operator<< failed");
    ...
    tr.run("copy reshape transpose T identity I", [] {
        ...
        assert_matrix_near(C, {{1.0L, 2.0L, 3.0L, 4.0L}}, 1e-12L,
                           "reshape(pair)");
        ...
        Matrix AT = A.T();
        assert_matrix_near(At, {{1.0L, 3.0L}, {2.0L, 4.0L}}, 1e-12L,
                           "transpose");
    ...
    tr.run("Symmetry and sums", [] {
        Matrix S({{1.0L, 2.0L, 3.0L}, {4.0L, 5.0L, 6.0L}}, 1e-10L,
                 DType::Float);
    ...
    tr.run("Power functions", [] {
        ...
        assert_matrix_near(p2, {{4.0L, 0.0L}, {0.0L, 9.0L}}, 1e-12L,
                           "pow alias");
    });
    ...
    tr.run("LU decompositions and wrappers", [] {
        ...
        assert_matrix_near(P * A, (L * U).to_float_data(), 1e-6L,
                           "PLU relation");
    ...
    tr.run("Gram-Schmidt and QR families", [] {
        Matrix A({{1.0L, 1.0L}, {1.0L, -1.0L}, {1.0L, 0.0L}}, 1e-10L,
                 DType::Float);
    ...
    tr.run("dominantEigen and fullRankDecomposition", [] {
        ...
        long double nrm =
            std::sqrt(vec(0, 0) * vec(0, 0) + vec(1, 0) * vec(1, 0));
