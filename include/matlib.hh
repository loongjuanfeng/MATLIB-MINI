...
class Rational {
   ...
   public:
    ...
    static Rational fromLongDouble(long double value,
                                   std::int64_t max_den = 1000000) {
        if (!std::isfinite(static_cast<double>(value))) {
            throw std::invalid_argument(
                "Cannot cast non-finite value to Rational.");
        }
        ...
        long double scaled =
            std::round(value * static_cast<long double>(max_den));
...
class Matrix {
   public:
    class CellProxy {
       ...
       public:
        ...
        operator long double() const { return owner_.at_float(row_, col_); }

        operator Rational() const { return owner_.at_fraction(row_, col_); }
    };
   ...
   private:
    ...
    static long double dot(const std::vector<long double>& v1,
    ...
    static std::vector<long double> normalize_vec(
        const std::vector<long double>& vec, long double tol) {
    ...
    static std::vector<std::vector<long double>> complete_orthonormal_basis(
        ...
        std::size_t dimension, long double tol) {
        ...
        if (basis.size() < dimension) {
            throw std::runtime_error(
                "Failed to complete an orthonormal basis.");
        }
        return basis;
    }
    ...
    static Rational evaluate_poly(const std::vector<Rational>& poly,
                                  const Rational& x) {
        Rational res(0);
    ...
    static std::vector<Rational> synthetic_div(
        const std::vector<Rational>& poly, const Rational& root) {
    ...
    Matrix to_fraction_matrix() const {
        ...
        std::vector<std::vector<Rational>> data(
            rows_, std::vector<Rational>(cols_, Rational(0)));
   ...
   public:
    ...
    Matrix(std::size_t rows, std::size_t cols, long double init_value = 0.0L,
           long double tolerance = 1e-10L, DType dtype = DType::Fraction)
        ...
        if (dtype_ == DType::Fraction) {
            ...
            float_data_.assign(rows_,
                               std::vector<long double>(cols_, init_value));
        }
    }

    Matrix(const std::vector<std::vector<long double>>& data,
           long double tolerance = 1e-10L, DType dtype = DType::Fraction)
        : rows_(data.size()),
        ...
        for (const auto& row : data) {
            if (row.size() != cols_) {
                throw std::invalid_argument(
                    "All rows in data must have the same length.");
            }
        }
    ...
    std::vector<std::vector<long double>> to_float_data() const {
        std::vector<std::vector<long double>> out(
            rows_, std::vector<long double>(cols_, 0.0L));
        for (std::size_t i = 0; i < rows_; ++i) {
            for (std::size_t j = 0; j < cols_; ++j) {
    ...
    static std::string method_comparison() {
        return "# Method Comparison\n"
               "- Gaussian elimination: partial pivoting is more stable than "
               "no-pivot elimination; use pivoting by default.\n"
               "- QR decomposition: Modified Gram-Schmidt is usually better "
               "than "
               "Classical Gram-Schmidt in finite precision.\n"
               "- QR with Householder is typically the most numerically stable "
               "general-purpose QR.\n"
               "- Eigenvalues: Jacobi is preferred for real symmetric matrices "
               "(stable, gives orthonormal eigenvectors).\n"
               "- Eigenvalues: QR iteration is more general for non-symmetric "
               "matrices, but eigenvectors are not returned here.\n"
               "- SVD: using A^T A for V and then U = A v_i / sigma_i is more "
               "robust than independently matching U from A A^T.\n"
               "- Cholesky is best for symmetric positive-definite matrices; "
               "LU is "
               "more general.";
    }
    ...
    Matrix reshape(const std::pair<std::size_t, std::size_t>& newdim) const {
        ...
        if (rows_ * cols_ != nr * nc) {
            throw std::invalid_argument(
                "Cannot reshape to a different total number of elements.");
        }
        ...
        if (dtype_ == DType::Fraction) {
            ...
            std::vector<std::vector<Rational>> out(
                nr, std::vector<Rational>(nc, Rational(0)));
            std::size_t idx = 0;
        ...
        std::vector<std::vector<long double>> out(
            nr, std::vector<long double>(nc, 0.0L));
        std::size_t idx = 0;
        for (std::size_t i = 0; i < nr; ++i) {
            for (std::size_t j = 0; j < nc; ++j) {
                out[i][j] = flat[idx++];
    ...
    Matrix transpose() const {
        if (dtype_ == DType::Fraction) {
            std::vector<std::vector<Rational>> out(
                cols_, std::vector<Rational>(rows_, Rational(0)));
            for (std::size_t i = 0; i < rows_; ++i) {
                for (std::size_t j = 0; j < cols_; ++j) {
        ...
        std::vector<std::vector<long double>> out(
            cols_, std::vector<long double>(rows_, 0.0L));
        for (std::size_t i = 0; i < rows_; ++i) {
            for (std::size_t j = 0; j < cols_; ++j) {
    ...
    static Matrix identity(std::size_t n, long double tolerance = 1e-10L,
    ...
    Matrix sum(const std::optional<int>& axis = std::nullopt) const {
        ...
        if (!axis.has_value()) {
            if (dtype_ == DType::Fraction) {
                ...
                return Matrix(std::vector<std::vector<Rational>>{{total}},
                              tolerance_);
            }
            ...
            return Matrix(std::vector<std::vector<long double>>{{total}},
                          tolerance_, DType::Float);
        }

        if (axis.value() == 0) {
            if (dtype_ == DType::Fraction) {
                std::vector<std::vector<Rational>> out(
                    1, std::vector<Rational>(cols_, Rational(0)));
                for (std::size_t j = 0; j < cols_; ++j) {
                    Rational col_sum(0);
                    for (std::size_t i = 0; i < rows_; ++i) {
            ...
            std::vector<std::vector<long double>> out(
                1, std::vector<long double>(cols_, 0.0L));
            for (std::size_t j = 0; j < cols_; ++j) {
                for (std::size_t i = 0; i < rows_; ++i) {
        ...
        if (dtype_ == DType::Fraction) {
            std::vector<std::vector<Rational>> out(
                rows_, std::vector<Rational>(1, Rational(0)));
            for (std::size_t i = 0; i < rows_; ++i) {
                Rational row_sum(0);
                for (std::size_t j = 0; j < cols_; ++j) {
        ...
        std::vector<std::vector<long double>> out(
            rows_, std::vector<long double>(1, 0.0L));
        for (std::size_t i = 0; i < rows_; ++i) {
            for (std::size_t j = 0; j < cols_; ++j) {
    ...
    Matrix concatenate(const Matrix& other, int index = 0) const {
        DType out_dtype =
            (dtype_ == DType::Fraction && other.dtype_ == DType::Fraction)
                ? DType::Fraction
                : DType::Float;
        ...
        if (index == 0) {
            if (cols_ != other.cols_) {
                throw std::invalid_argument(
                    "For row concatenation, column counts must match.");
            }
        ...
        if (index == 1) {
            if (rows_ != other.rows_) {
                throw std::invalid_argument(
                    "For column concatenation, row counts must match.");
            }

            if (out_dtype == DType::Fraction) {
                std::vector<std::vector<Rational>> out(
                    rows_,
                    std::vector<Rational>(cols_ + other.cols_, Rational(0)));
                for (std::size_t i = 0; i < rows_; ++i) {
                    for (std::size_t j = 0; j < cols_; ++j) {
            ...
            std::vector<std::vector<long double>> out(
                rows_, std::vector<long double>(cols_ + other.cols_, 0.0L));
            for (std::size_t i = 0; i < rows_; ++i) {
                for (std::size_t j = 0; j < cols_; ++j) {
                    out[i][j] = at_float(i, j);
                }
    ...
    Matrix kronecker_product(const Matrix& other) const {
        ...
        if (out_dtype == DType::Fraction) {
            std::vector<std::vector<Rational>> out(
                out_rows, std::vector<Rational>(out_cols, Rational(0)));
            for (std::size_t i = 0; i < rows_; ++i) {
                for (std::size_t j = 0; j < cols_; ++j) {
        ...
        std::vector<std::vector<long double>> out(
            out_rows, std::vector<long double>(out_cols, 0.0L));
        for (std::size_t i = 0; i < rows_; ++i) {
            for (std::size_t j = 0; j < cols_; ++j) {
                for (std::size_t p = 0; p < other.rows_; ++p) {
                    for (std::size_t q = 0; q < other.cols_; ++q) {
    ...
    void print(int precision = 4) const { std::cout << str(precision) << "\n"; }
    ...
    Matrix operator*(long long scalar) const { return num_product(scalar); }

    Matrix operator*(long double scalar) const { return num_product(scalar); }
    ...
    Matrix operator*(const Matrix& other) const {
        ...
        if (out_dtype == DType::Fraction) {
            std::vector<std::vector<Rational>> out(
                rows_, std::vector<Rational>(other.cols_, Rational(0)));
            for (std::size_t i = 0; i < rows_; ++i) {
                for (std::size_t j = 0; j < other.cols_; ++j) {
                    Rational val(0);
        ...
        std::vector<std::vector<long double>> out(
            rows_, std::vector<long double>(other.cols_, 0.0L));
        for (std::size_t i = 0; i < rows_; ++i) {
            for (std::size_t j = 0; j < other.cols_; ++j) {
    ...
    Matrix power(int n) const {
        if (n < 0) {
            throw std::invalid_argument(
                "Negative powers are not supported directly; use inverse() "
                "first.");
        }
        if (rows_ != cols_) {
            throw std::invalid_argument(
    ...
    Matrix pow(int n) const { return power(n); }
                ...
                for (std::size_t i = r; i < rows_; ++i) {
                    if (mat.dtype_ == DType::Fraction) {
                        if (!near_zero(mat.frac_data_[i][c], mat.tolerance_)) {
                            long double score =
                                std::fabs(mat.frac_data_[i][c].toLongDouble());
                            if (score > best) {
                                best = score;
                                pivot_row = i;
                            }
    ...
    long double det() const { return determinant(); }
    ...
    Matrix inverse() const {
        ...
        if (dtype_ == DType::Fraction) {
            std::vector<std::vector<Rational>> aug(
                n, std::vector<Rational>(2 * n, Rational(0)));
            for (std::size_t i = 0; i < n; ++i) {
            ...
            std::vector<std::vector<Rational>> inv(
                n, std::vector<Rational>(n, Rational(0)));
            for (std::size_t i = 0; i < n; ++i) {
                for (std::size_t j = 0; j < n; ++j) {
                    inv[i][j] = aug[i][j + n];
        ...
        std::vector<std::vector<long double>> aug(
            n, std::vector<long double>(2 * n, 0.0L));
        for (std::size_t i = 0; i < n; ++i) {
            for (std::size_t j = 0; j < n; ++j) {
    ...
    std::tuple<Matrix, Matrix, Matrix> lu_decomposition(
        ...
        if (dtype_ == DType::Fraction) {
            ...
            std::vector<std::vector<Rational>> L(
                n, std::vector<Rational>(n, Rational(0)));
            ...
            for (std::size_t k = 0; k < n; ++k) {
                ...
                if (near_zero(U[pivot_row][k], tolerance_)) {
                    throw std::runtime_error(
                        "Matrix is singular or LU requires pivoting at a zero "
                        "pivot.");
                }
            ...
            return {Matrix(P, tolerance_), Matrix(L, tolerance_),
                    Matrix(U, tolerance_)};
        }
        ...
        return {Matrix(P, tolerance_, DType::Float),
    ...
    Matrix gram_schmidt(const std::string& method = "mgs") const {
        if (rows_ == 0 || cols_ == 0) {
            throw std::invalid_argument(
                "Cannot run Gram-Schmidt on an empty matrix.");
        }
        ...
        if (method_key != "cgs" && method_key != "classical" &&
            ...
            throw std::invalid_argument(
                "method must be 'cgs'/'classical' or 'mgs'/'modified'.");
        }

        auto A = to_float_data();
        std::vector<std::vector<long double>> cols(
            cols_, std::vector<long double>(rows_, 0.0L));
        for (std::size_t j = 0; j < cols_; ++j) {
            for (std::size_t i = 0; i < rows_; ++i) {
                cols[j][i] = A[i][j];
        ...
        if (method_key == "cgs" || method_key == "classical") {
            for (const auto& col : cols) {
                ...
                if (nrm <= tolerance_) {
                    throw std::runtime_error(
                        "Columns are linearly dependent; CGS failed.");
                }
                for (long double& x : v) {
                    x /= nrm;
                }
            ...
            for (const auto& col : cols) {
                ...
                if (nrm <= tolerance_) {
                    throw std::runtime_error(
                        "Columns are linearly dependent; MGS failed.");
                }
                for (long double& x : v) {
                    x /= nrm;
                }
        ...
        std::vector<std::vector<long double>> q_data(
            rows_, std::vector<long double>(cols_, 0.0L));
        for (std::size_t j = 0; j < cols_; ++j) {
            for (std::size_t i = 0; i < rows_; ++i) {
    ...
    std::pair<Matrix, Matrix> _qr_householder() const {
        ...
        auto Q =
            Matrix::identity(rows_, tolerance_, DType::Float).to_float_data();
        ...
        return {Matrix(Q, tolerance_, DType::Float),
                Matrix(R, tolerance_, DType::Float)};
    }

    std::pair<Matrix, Matrix> qr_decomposition(
        const std::string& method = "householder") const {
        std::string method_key = to_lower(method);
        ...
        if (method_key == "mgs" || method_key == "modified" ||
            ...
            return {Q, Matrix(r_data, tolerance_, DType::Float)};
        }
    ...
    std::vector<long double> eigenvalues_qr(
        int iterations = 100,
        const std::string& qr_method = "householder") const {
        if (rows_ != cols_) {
            throw std::invalid_argument(
    ...
    std::pair<std::vector<long double>, Matrix> eigenpairs_jacobi(
        int max_iterations = 1000, bool sort = true) const {
        if (rows_ != cols_) {
            throw std::invalid_argument(
                "Eigenvalues are defined only for square matrices.");
        }
        if (!_is_symmetric()) {
            throw std::invalid_argument(
                "Jacobi method requires a symmetric matrix.");
        }
        ...
        for (int iter = 0; iter < max_iterations; ++iter) {
            ...
            long double t =
                sign_tau / (std::fabs(tau) + std::sqrt(tau * tau + 1.0L));
        ...
        if (sort) {
            ...
            std::vector<std::vector<long double>> sorted_V(
                n, std::vector<long double>(n, 0.0L));
            for (std::size_t i = 0; i < n; ++i) {
    ...
    std::pair<std::vector<long double>, Matrix> eigenpairs(
        const std::string& method = "auto", int iterations = 100,
        ...
        if (method_key == "qr") {
            return {eigenvalues_qr(iterations, qr_method), Matrix()};
        }

        throw std::invalid_argument(
            "method must be 'auto', 'jacobi', or 'qr'.");
    }
    ...
    std::tuple<Matrix, Matrix, Matrix> singular_value_decomposition() const {
        ...
        Matrix AtA((this->transpose() * (*this)).to_float_data(), tolerance_,
                   DType::Float);
        ...
        std::vector<std::vector<long double>> sigma_data(
        ...
        std::vector<std::vector<long double>> v_cols(
            n, std::vector<long double>(n, 0.0L));
        ...
        std::vector<std::vector<long double>> U_data(
    ...
    Matrix jordan_normal_form() const {
        if (rows_ != cols_) {
            throw std::invalid_argument(
                "Jordan normal form is defined only for square matrices.");
        }
        if (rows_ == 0) {
            return Matrix();
        }
        ...
        Matrix M_mat(std::vector<std::vector<Rational>>(
                         n, std::vector<Rational>(n, Rational(0))),
                     tolerance_);
        ...
        for (std::size_t k = 1; k <= n; ++k) {
            ...
            std::vector<std::vector<Rational>> c_prev_I(
                n, std::vector<Rational>(n, Rational(0)));
            for (std::size_t i = 0; i < n; ++i) {
                c_prev_I[i][i] = c_prev;
            }
        ...
        if (poly.size() > 1) {
            ...
            for (const auto& coeff : poly) {
                int_coeffs.push_back(Rational(
                    coeff.numerator() * (lcm_den / coeff.denominator()), 1));
            }
            ...
            for (const auto& entry : candidates) {
                ...
                while (int_coeffs.size() > 1 &&
                       evaluate_poly(int_coeffs, cand).isZero()) {
                    roots[cand] += 1;
            ...
            if (int_coeffs.size() > 1) {
                throw std::runtime_error(
                    "Matrix has non-rational eigenvalues; exact Jordan form is "
                    "unsupported here.");
            }
        }
        ...
        for (const auto& root : roots) {
            ...
            std::vector<std::vector<Rational>> diag_B_data(
                n, std::vector<Rational>(n, Rational(0)));
            for (std::size_t i = 0; i < n; ++i) {
            ...
            for (int k = 1; k <= alg_mult + 1; ++k) {
                ...
                if (ranks.size() >= 3 &&
                    ranks[ranks.size() - 1] == ranks[ranks.size() - 2]) {
        ...
        std::vector<std::vector<Rational>> J_data(
            n, std::vector<Rational>(n, Rational(0)));
        std::size_t idx = 0;
        for (const auto& block : jordan_blocks) {
            ...
            for (int i = 0;
                 i < block_size && idx + static_cast<std::size_t>(i) < n; ++i) {
                J_data[idx + static_cast<std::size_t>(i)]
                      [idx + static_cast<std::size_t>(i)] = eigenvalue;
                if (i < block_size - 1 &&
                    idx + static_cast<std::size_t>(i + 1) < n) {
                    J_data[idx + static_cast<std::size_t>(i)]
                          [idx + static_cast<std::size_t>(i + 1)] = Rational(1);
                }
            }
    ...
    Matrix jordanNormalForm() const { return jordan_normal_form(); }

    void dominantEigen(long double& eigenvalue, Matrix& eigenvector,
                       int iterations = 1000) const {
        if (rows_ != cols_) {
        ...
        for (int it = 0; it < iterations; ++it) {
            ...
            if (nrm <= tolerance_) {
                throw std::runtime_error(
                    "Power iteration failed due to near-zero norm.");
            }
            for (std::size_t i = 0; i < rows_; ++i) {
    ...
    void fullRankDecomposition(Matrix& F, Matrix& G) const {
        ...
        for (std::size_t r = 0; r < rows_; ++r) {
            ...
            std::size_t i = r;
            while (i < rows_ &&
                   std::fabs(rref.at_float(i, lead)) <= tolerance_) {
                ++i;
            }
            if (i == rows_) {
                ++lead;
            ...
            for (std::size_t i2 = 0; i2 < rows_; ++i2) {
                ...
                for (std::size_t c = 0; c < cols_; ++c) {
                    rref.set_value(
                        i2, c,
                        rref.at_float(i2, c) - lv2 * rref.at_float(r, c));
                }
            }
...
Matrix I(std::size_t n, long double tolerance = 1e-10L,
         const std::string& dtype = "fraction") {
